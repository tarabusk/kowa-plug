...
add_action('wp_head',  'azalyne_add_analytics_script'); 
...
function add_anchor_to_title($content){
  //global $post;
  //setup_postdata( $post );
//  if($post->post_type = 'post'){ // s'il s'agit d'un article
  if (is_single ()) {
    //global $post;
...
function kowala_automenu($atts){
  ...
  // echo get_post_type ();
	...
	if (get_post_type () == 'post' && (($a['insertmobile'] == false) || wp_is_mobile())) {
	  $obj1 = '<ul id="kowala-sommaire">';
	  $original_content = get_post_field('post_content', $GLOBALS['post']->ID);
...
/********************************************/
/* Envoi un email lorsqu'un article est mis en attente de re-lecture */
/********************************************/

add_action( 'transition_post_status', 'kowala_pending_post_status', 10, 3 );

function kowala_pending_post_status( $new_status, $old_status, $post ) {

    if ( $new_status === "pending" && $old_status !== "pending" ) {
			 $headers = array('Content-Type: text/html; charset=UTF-8');
        	wp_mail('contact@kowala.fr',
									"Article en attente de re-lecture sur kowala.fr",
									"L'article suivant est en attente de re-lecture : <a href=". get_post_permalink ($post->ID). "> ".get_the_title($post->ID)."</a>",
									$headers);
